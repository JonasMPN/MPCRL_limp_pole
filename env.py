...
class CartPoleEnv(gym.Env):
    ...
    def step(self, action: np.ndarray, refs: list[float | str]) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if len(self.x_trajectory) >= 10:
            refs_achieved = [False, False, False, False]
            for i_ref, ref in enumerate(refs):
                if ref == "pass":
                    refs_achieved[i_ref] = True
                else:
                    refs_achieved[i_ref] = all(np.abs(self.x_trajectory[i][i_ref] - ref) < 0.1 for i in range(-10, 0))
            success = all(refs_achieved)
            trunc = success
        else:
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None) -> tuple[np.ndarray, dict]:  # type: ignore
        if seed is not None:
        ...
        if self.pos_trajectory is not None:
            ...
            for i, plan_pole_end in enumerate(plan_pole_end):
                if abs(plan_pole_end[0]) + r > self.screen_width:
                    # Dont render out of bounds
                    continue
                gfxdraw.filled_circle(canvas, int(plan_pole_end[0]), int(plan_pole_end[1]), r, (5, 255, 5))
